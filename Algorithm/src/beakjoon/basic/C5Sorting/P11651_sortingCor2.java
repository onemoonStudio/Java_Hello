...
public class P11651_sortingCor2 {
    public static void main(String[] args) throws Exception{
        ...
        quickSort(coordinate,0,howMany-1,1);
        int sub_st=0 , sub_end=0;

        for(int i=0;i<howMany;i++){
            if(coordinate[i][1] != coordinate[sub_st][1] ){
                sub_end = i-1;
                if(sub_end-sub_st > 0){
                    quickSort(coordinate,sub_st,sub_end,0);
                }
                sub_st = i; sub_end = i;
            }
            else if(i == howMany-1 && i-sub_st > 0){
                quickSort(coordinate,sub_st,i,0);
            }
